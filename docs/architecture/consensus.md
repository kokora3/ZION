...
The initial validator set comes only from bound genesis configuration. Runtime validator-set changes remain disabled until ZION governance defines authorization. General bootstrap, discovery, DHT, NAT traversal, relaying, and normal-node P2P are also outside Phase 5; CometBFT validator transport is not the future ZION content network.
