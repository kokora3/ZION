...
Cryptographic verification is not current authorization: canonical chain state determines active and retired keys and enforces rotation sequence. Membership is separately chain-governed (PENDING, ACTIVE, SUSPENDED, REVOKED); a valid identity or node is not automatically a member or validator. Phase 6 permits membership transitions only through canonical proposal execution. ZION member identities are not P2P Peer IDs.
