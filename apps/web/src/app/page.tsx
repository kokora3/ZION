    ...
    <NodeData loading={view.loading} error={view.error} empty={!status} retry={view.reload}>
      {status && <>
        ...
        <section className="grid" style={{marginTop:"1rem"}}>
          <article className="card"><h2>Canonical application</h2><dl className="stat-list"><dt>StateHash</dt><dd><Identifier value={status.state_hash} label="StateHash" /></dd><dt>Protocol</dt><dd>{status.protocol_version}</dd><dt>Validator authority</dt><dd>{status.validator_authorized ? "Authorized by canonical/genesis state" : "Not authorized"}</dd><dt>Consensus process</dt><dd>{status.consensus_active ? "Active" : "Inactive"}</dd></dl></article>
